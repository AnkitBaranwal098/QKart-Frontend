...
const Checkout = () => {
  ...
  //In all we store an array containing a list of all addressed and in selected we basically store the selected addressId
  ...
  const validateRequest = (items, addresses) => {
    const walletAmt = Number(localStorage.getItem("balance"));
    const purchaseAmt = getTotalCartValue(items);
    if (walletAmt - purchaseAmt <= 0) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        { variant: "warning" }
      );
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      const newBalance =
        Number(localStorage.getItem("balance")) - getTotalCartValue(items);
  ...
  const checkIfLoggedIn = async (token) => {
    console.log("Inside method checkIfLoggedIn");
    if (token) {
      setIsLoggedIn(true);
      const xyz = await getAddresses(token);
      console.log(xyz);
      console.log("User is Logged in !!!");
    } else {
      console.log("You must be logged in to access checkout page");
      setIsLoggedIn(false);
