...
const Login = () => {
  ...
  const[loginLoader,setLogin] = useState(200);
    ...
    if (validateInput()) {
      ...
      try {
        ...
        if (error.response && error.response.status === 400) {
          setLogin(200);
          return triggerSnackbar(error.response.data.message, "error");
        } else {
          setLogin(200);
    ...
    >
      ...
      <Box className="content">
        <Stack spacing={2} className="form">
          ...
          />
          {loginLoader===200 && ( <Button className="button" variant="contained" onClick={login}>
            LOGIN TO QKART
          </Button>)}
          {loginLoader===202 && ( <Box sx={{ display: "flex", justifyContent: "center" }}>
              <CircularProgress />
            </Box>)}
